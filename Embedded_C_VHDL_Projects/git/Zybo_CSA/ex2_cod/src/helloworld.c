...
int main()
{
   ...
   XGpio inputCsaA, inputCsaB, inputCin, outputSum, outputCout;
   ...
   XGpio_Initialize(&inputCsaA, XPAR_AXI_GPIO_A_BASEADDR);
   XGpio_Initialize(&inputCsaB, XPAR_AXI_GPIO_B_BASEADDR);
...
//acestea sunt pt CLA
   XGpio_SetDataDirection(&inputCsaA, 1, 0x0);			
   XGpio_SetDataDirection(&inputCsaB, 1, 0x0);			
   ...
   XGpio_SetDataDirection(&outputCout, 1, 1);			
    ...
    while(1){
      ...
      {
          XGpio_DiscreteWrite(&inputCsaA, 1, operandX);
          XGpio_DiscreteWrite(&inputCsaB, 1, operandY);
